...
var { TransactionUnspentOutput,
  ...
  function mergeOutputsAmounts(outputs) {
    let compiledAmountList = S.Value.new(
      S.BigNum.from_str('0')
    );
  ...
  function splitAmounts(amounts) {
    ...
    if (amounts.multiasset()) {
      ...
      for (let i = 0; i < mA.keys().len(); i++) {
        ...
        for (let j = 0; j < mA.get(scriptHash).keys().len(); j++) {
          let _assets = S.Assets.new();
          let assetName = mA.get(scriptHash).keys().get(j);
  
          _assets.insert(
            S.AssetName.from_bytes(assetName.to_bytes()),
            S.BigNum.from_bytes(
              mA.get(scriptHash).get(assetName).to_bytes()
            )
          );
  
          let _multiasset = S.MultiAsset.new();
          _multiasset.insert(
            S.ScriptHash.from_bytes(scriptHash.to_bytes()),
            _assets
          );
          let _value = S.Value.new(
            S.BigNum.from_str('0')
          );
    ...
    splitAmounts.push(
      S.Value.new(
        S.BigNum.from_bytes(amounts.coin().to_bytes())
      )
    );
  ...
  function cloneUTxOList(utxoList) {
    return utxoList.map((utxo) => {
      return S.TransactionUnspentOutput.from_bytes(utxo.to_bytes())
    }
    );
  }
  
  function cloneValue(value) { return S.Value.from_bytes(value.to_bytes()); }
  ...
  class CoinSelection {
    ...
    randomImprove(inputs, outputs, limit) {
      ...
      let utxoSelection = {
        ...
        subset: [],
        amount: S.Value.new(S.BigNum.from_str('0')),
      };
      ...
      for (let i = 0; i < splitOutputsAmounts.length; i++) {
        ...
        let range = {};
        range.ideal = S.Value.new(
          S.BigNum.from_str('0')
        )
          .checked_add(splitOutputsAmounts[i])
          .checked_add(splitOutputsAmounts[i]);
        range.maximum = S.Value.new(
          S.BigNum.from_str('0')
        )
      ...
      let minAmount = S.Value.new(
        S.min_ada_required(
          change,
          S.BigNum.from_str(this.protocolParameters.minUTxO)
        )
      );
      ...
      maxFee = S.Value.new(
        S.BigNum.from_str(maxFee.toString())
      );
      ...
      if (compare(change, minAmount) < 0) {
        ...
        const minAda = minAmount
          .checked_sub(S.Value.new(change.coin()))
          .checked_add(S.Value.new(utxoSelection.amount.coin()));
  
        createSubSet(utxoSelection, minAda);
        // change selecct -> this.selecct
        utxoSelection = this.select(utxoSelection, minAda, limit, _minUTxOValue);
      }
    ...
    isQtyFulfilled(
      ...
      if (minUTxOValue && BigInt(outputAmount.coin().to_str()) > 0) {
        let minAmount = S.Value.new(
          S.min_ada_required(
            cumulatedAmount,
            S.BigNum.from_str(minUTxOValue.toString())
          )
        );
