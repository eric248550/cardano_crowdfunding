...
// const CoinSelection = require('./coinSelection.js').CoinSelection;
const CoinSelection = require("./coinSelection").CoinSelection;
...
class NamiWalletApi {
    ...
    // generate keys Cardano from bech32 PrivateKey
    keysFrombech32PrivateKey(bech32PrivateKey, accountIndex=0) {
        ...
        const privateKey = S.Bip32PrivateKey.from_bech32(bech32PrivateKey)
        ...
        .derive(0) // external
        .derive(0);//index 1...n to get more payment keys

        const stakeKey = accountKey
        .derive(2) // chimeric
        .derive(0);// index 1 onwards to get more stake keys

        // const baseAddr = S.BaseAddress.new(
        //     S.NetworkInfo.mainnet().network_id(),
        //     S.StakeCredential.from_keyhash(paymentKey.to_raw_key().hash()),
        //     S.StakeCredential.from_keyhash(stakeKey.to_raw_key().hash()),
        // );

        // const baseAddr_testnet = S.BaseAddress.new(
        ...
        return {
            ...
            "stakeKey": stakeKey,
            // "address_mainnet": baseAddr.to_address().to_bech32(),
    ...
    addressesFromPrivateKey(ptivateKey, accountIndex=0) {
        const accountKey = ptivateKey
            .derive(harden(1852)) // purpose
            .derive(harden(1815)) // coin type
            .derive(harden(accountIndex)); // account #0
        ...
        const baseAddr = S.BaseAddress.new(
            S.NetworkInfo.mainnet().network_id(),
            S.StakeCredential.from_keyhash(utxoPubKey.to_raw_key().hash()),
            S.StakeCredential.from_keyhash(stakeKey.to_raw_key().hash()),
        );
    ...
    async addressHextoBech32(usedAddresses) {

        const addressHex = Buffer.from(usedAddresses, "hex");
    ...
    async getBalance(valueCBOR, networdId) {

        let protocolParameter = await this._getProtocolParameter(networdId);
        ...
        let countedValue = S.Value.new(S.BigNum.from_str("0"));
    ...
    // covert keyhash and slot to policyScript 
    async importPolicyScript(slot, scriptKeyHash) {

        const fromHexString = hexString =>
            new Uint8Array(hexString.match(/.{1,2}/g).map(byte => parseInt(byte, 16)));       
        
        const keyHash = S.Ed25519KeyHash.from_bytes(fromHexString(scriptKeyHash));
        ...
        return {
            ...
            keyHash: Buffer.from(keyHash.to_bytes(), "hex").toString("hex"),
            slot
        };
    }
    // create policy
    ...
    async decodeTransaction(transactionHex, networkId ) {
        ...
        let _metadata;
        if (auxiliary_data) {
            _metadata = auxiliary_data.metadata()
        }
        ...
        Object.keys(txInputs).map((senderAddress) => {
            // 不知道為何 先把它改成 "undefine" -> undefine
            
            if (recipients[senderAddress] != undefined && recipients[senderAddress] != "undefined") {
                ...
                Object.entries(recipients[senderAddress].assets).forEach(([unit, quantity]) => {
                    ...
                    if (typeof txInputs[senderAddress].assets[unit] != undefined && typeof txInputs[senderAddress].assets[unit] != "undefined") {
    ...
    signTx(txHash, skey){ 
        ...
        // // sign with skey
        const witness = S.make_vkey_witness(txBodyHash, S.PrivateKey.from_normal_bytes(Buffer.from(skey, "hex")))
    ...
    signTxWithPkey(txHash, pkey){ 
        ...
        // // sith with pk
        // const witness = S.make_vkey_witness(txBodyHash, pkey )
        const witness = S.make_vkey_witness(txBodyHash, pkey.to_raw_key() )
    ...
    async submitTx({
        ...
        // console.log(Buffer.from(signedTx.to_bytes(), "hex"))
        // console.log(signedTx.to_bytes())
        // console.dir(signedTx.to_bytes(), {'maxArrayLength': null})
    ...
    // convert UTxO response from blockfrost to UTxO list
    ...
    // convert metadata to hash (multisig)
    ...
    async _getProtocolParameter(networkId) {
        ...
        return {
            linearFee: {
                ...
                // add fee manually
                minFeeB: (p.min_fee_b + 10000).toString(),
            },
